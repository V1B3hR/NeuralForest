...
class CrossGroveAttention(nn.Module):
    ...
    def forward(
        self, grove_outputs: torch.Tensor, grove_mask: Optional[torch.Tensor] = None
        ...
        attn_output, attn_weights = self.attention(
            grove_outputs, grove_outputs, grove_outputs, key_padding_mask=grove_mask
        )
    ...
    def forward_with_weights(
        self, grove_outputs: torch.Tensor, grove_weights: torch.Tensor
    ) -> torch.Tensor:
        """
...
class AdaptiveAggregator(nn.Module):
    ...
    def forward(self, grove_outputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.use_attention:
            ...
            # Use learned weights
            flattened = stacked.view(
                stacked.shape[0], -1
            )  # [B, num_groves * embed_dim]
    ...
    def get_aggregation_weights(
        self, grove_outputs: List[torch.Tensor]
    ) -> torch.Tensor:
        """
        ...
        if self.use_attention:
            ...
            return torch.ones(
                batch_size, len(grove_outputs), device=stacked.device
            ) / len(grove_outputs)
        else:
