...
class GroveRouter(nn.Module):
    ...
    """
    def __init__(self, num_groves: int = 4, embedding_dim: int = 512, hidden_dim: int = 128):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, active_groves: Optional[List[str]] = None) -> torch.Tensor:
        """
    ...
    def get_top_groves(self, x: torch.Tensor, top_k: int = 2) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class ForestCanopy(nn.Module):
    ...
    """
    def __init__(self, grove_dict: Optional[Dict[str, nn.Module]] = None, 
    ...
    def forward(self, x: torch.Tensor, 
                ...
                top_k_groves: int = 2) -> Tuple[torch.Tensor, Dict]:
        """
        ...
        if len(grove_outputs) == 1:
            ...
            stacked_weights = torch.stack(grove_weights_list, dim=1)  # [B, num_groves, 1]
    ...
    def _select_modalities(self, modality_probs: Dict[str, torch.Tensor], 
        ...
        if not selected:
            max_modality = max(modality_probs.items(), 
                             key=lambda x: x[1][0].item())
