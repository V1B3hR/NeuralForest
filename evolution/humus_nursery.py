"""
Humus Nursery — Natural Selection Archive for NeuralForest (Phase 1: Soil Purification)

This module implements the memory and archival system for trees removed through natural
selection, enabling post-mortem analysis, resurrection, and evolutionary insights.
Like organic humus in a real forest, archived trees decompose into rich soil that
nourishes future saplings.

Key Features:
- Archive naturally-selected trees with full metadata (architecture, weights, history, genealogy)
- Post-mortem analysis of eliminated trees
- Resurrection mechanism to reintroduce archived trees as new saplings
- Knowledge repository for evolutionary insights extracted from fallen trees
"""
...
@dataclass
class NurseryStats:
    """Statistics about the humus nursery (trees removed through natural selection)."""
...
GraveyardStats = NurseryStats


class HumusNursery:
    """
    Archive for trees removed through natural selection, with full analysis capabilities.
    
    Like the humus layer of a real forest — fallen trees decompose here, enriching
    the soil with knowledge that new saplings can absorb.
    
    Features:
    - Store complete tree records before natural selection removes them
    - Query and analyze naturally-selected trees
    - Resurrection mechanism for reintroducing promising trees as new saplings
    ...
    def __init__(
        ...
        """
        Initialize the Humus Nursery.
        
        Args:
            max_records: Maximum number of records to keep in memory
            save_weights: Whether to save model weights for naturally-selected trees
            weights_dir: Directory to save weights (if save_weights=True)
            auto_save: Automatically save nursery to disk periodically
            save_path: Path to save nursery JSON records
        """
    ...
    def archive_tree(
        ...
        logger.info(
            f"Archived tree {tree_id} into humus nursery: reason={elimination_reason}, "
    ...
    def save_to_disk(self, path: Optional[Path] = None) -> None:
        ...
        try:
            ...
            logger.info(f"Saved {len(self.records)} humus nursery records to {save_path}")
        except Exception as e:
    ...
    def load_from_disk(self, path: Optional[Path] = None) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to load humus nursery from disk: {e}")
    ...
    def clear(self) -> None:
        ...
        self.records_by_generation.clear()
        self.stats = NurseryStats()
        logger.warning("Humus nursery cleared - all records removed")


# Backward-compatibility alias: code that imports TreeGraveyard still works.
TreeGraveyard = HumusNursery
