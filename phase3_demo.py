...
def demo_modality_detector():
    ...
    for modality_name, features in modalities_to_test.items():
        ...
        print(f"  Probabilities: {', '.join([f'{k}: {v[0].item():.3f}' for k, v in probs.items()])}")
...
def demo_load_balancer():
    ...
    for tree_id, util in sorted(utilizations.items()):
        status = "overutilized" if util > 0.8 else "underutilized" if util < 0.3 else "balanced"
...
def demo_forest_canopy():
    ...
    print(f"  Grove routing probabilities:")
