...
class AutumnPruning:
    ...
    def evaluate_forest_health(self) -> Dict:
        ...
        if weak_count > 0:
            report["recommendations"].insert(
                0, f"Found {weak_count} weak tree(s). Pruning recommended."
            )
        if ancient_count > 0:
            report["recommendations"].append(
    ...
    def prune_weakest(self, config: Dict, min_keep: int = 3) -> List[int]:
        ...
        # Find weak trees
        weak_trees = [t for t in self.forest.trees if t.fitness < 2.0 and t.age > 40]
        ...
        max_to_remove = min(len(weak_trees) // 2 + 1, len(self.forest.trees) - min_keep)
        ...
        for tree in weakest:
            print(
                f"🍂 Autumn pruning: removing tree {tree.id} (fitness: {tree.fitness:.2f}, age: {tree.age})"
            )
    ...
    def get_pruning_recommendations(self) -> List[str]:
        ...
        if health["overall_health"] == "poor":
            recommendations.append(
                "⚠️ Forest health is poor. Aggressive pruning recommended."
            )
        elif health["overall_health"] == "excellent":
            recommendations.append(
                "✅ Forest health is excellent. Light maintenance pruning only."
            )
        ...
        if self.forest.trees:
            ...
            if avg_age > 200:
                recommendations.append(
                    "Trees are mature. Consider planting younger specialists."
                )
            elif avg_age < 50:
                recommendations.append(
                    "Trees are young. Give them time to mature before pruning."
                )

        if not recommendations:
            recommendations.append(
                "Forest is well-balanced. No immediate actions needed."
            )
