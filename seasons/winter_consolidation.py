...
class WinterConsolidation:
    ...
    def deep_consolidation(self, num_rounds: int = 10, batch_size: int = 64) -> Dict:
        ...
        if hasattr(self.forest, "anchors") and len(self.forest.anchors) > 0:
            ...
            for round_idx in range(num_rounds):
                try:
                    ax, ay = self.forest.anchors.sample(
                        batch_size=min(batch_size, len(self.forest.anchors))
                    )
            ...
            if anchor_losses:
                ...
                print(
                    f"  🔒 Anchors reinforced: avg loss = {results['anchor_loss']:.4f}"
                )
            ...
            for student in weak_trees:
                if teacher.id != student.id:
                    try:
                        ...
                        print(
                            f"  ⚠️ Transfer from tree {teacher.id} to {student.id} failed: {e}"
                        )
    ...
    def strengthen_bark(self) -> Dict:
        ...
        if bark_values_before:
            results["avg_bark_before"] = sum(bark_values_before) / len(
                bark_values_before
            )
    ...
    def consolidate_memory(self, priority_boost: float = 1.5) -> Dict:
        ...
        """
        results = {"memories_boosted": 0, "total_memories": 0}
    ...
    def get_consolidation_metrics(self) -> Dict:
        ...
        if hasattr(self.forest, "trees") and self.forest.trees:
            ...
            metrics["avg_bark"] = sum(t.bark for t in self.forest.trees) / len(
                self.forest.trees
            )
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if metrics.get("consolidation_rounds", 0) == 0:
            recommendations.append(
                "❄️ No consolidation performed yet. Run deep_consolidation()."
            )

        # Check knowledge transfer
        if metrics.get("total_transfers", 0) == 0:
            recommendations.append(
                "No knowledge transfers performed. Trees may benefit from cross-tree learning."
            )
        ...
        if mulch_size > 5000:
            recommendations.append(
                "Mulch is large. Consider pruning old memories to save resources."
            )
        if anchor_size < 100:
            recommendations.append(
                "Few anchor memories. Consider adding more to prevent forgetting."
            )
        ...
        if avg_bark < 0.5:
            recommendations.append(
                "Low bark protection. Strengthen bark to protect knowledge."
            )
