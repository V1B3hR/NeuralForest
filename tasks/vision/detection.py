...
class ObjectDetection(TaskHead):
    ...
    SUPPORTED_DATASETS = ["coco", "pascal_voc", "open_images", "objects365"]

    def __init__(
        self, input_dim: int, num_classes: int, task_name: str = "object_detection"
    ):
        """
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        return {"bbox": bbox, "class_logits": class_logits, "confidence": confidence}
    ...
    def get_loss(self, predictions: dict, targets: dict) -> torch.Tensor:
        ...
        bbox_loss = nn.functional.l1_loss(predictions["bbox"], targets["target_bbox"])

        # Classification loss
        class_loss = nn.functional.cross_entropy(
            predictions["class_logits"], targets["target_classes"]
        )

        # Confidence loss (binary cross entropy)
        # Confidence should be high for correct predictions
        conf_targets = (
            (predictions["class_logits"].argmax(dim=-1) == targets["target_classes"])
            .float()
            .unsqueeze(1)
        )
        conf_loss = nn.functional.binary_cross_entropy(
            predictions["confidence"], conf_targets
        )
